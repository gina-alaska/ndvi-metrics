...
pro interpol_nonextension_vector, v, v_bname, threshold,snowcld,v_interp,v_bname_interp,ratio,flg_metrics
...
;output: v_interp---- result vector, v_bname_interp --- result of band names, flg_metrics =0, not calculate, 1==suitable for metrics calculation
...
;--------- determind if the vector is suitable for metrics calculation

numofmidyr=n_elements(v)

idxv=where( (v NE threshold) and (v NE snowcld) and (v GT 100),cntv )
